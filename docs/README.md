...
- **Fluxo de Entrevista em 6 Etapas**: `intro → termos_aceite → momento_carreira → expectativas_evento → objetivo_principal → finalizacao`
...
```
intro → termos_aceite → momento_carreira → expectativas_evento → objetivo_principal → finalizacao
```

#### Detalhes das Etapas

1. **intro**: Coleta de consentimento e boas-vindas
2. **termos_aceite**: Confirmação de termos e validação de dados
